...
def logic() -> str:
    ...
    prefix = arrow.get(CURR_PERIOD, "MMMYY").shift(
        months=+2).format("YYYY/MM") + '/(?:DM01|Monthly-Diagnostics)'

    if REVISIONS:
        # revision files should have the year month of CURR_PERIOD ( + 2 months due to delay between data for and reporting date) in the url as an assumption e.g. 2021/06

        return "|".join(
            [f'{prefix}.*-{arrow.get(CURR_PERIOD, "MMMYY").format("MMMM-YYYY")}', '{}.*({})'.format(prefix, "|".join(month_list))])  # create the regex expression used to match urls on website.

    return f'{prefix}.*-{arrow.get(CURR_PERIOD, "MMMYY").format("MMMM-YYYY")}'
...
async def get_links(url, nurse):
    async with httpx.AsyncClient(timeout=None) as client:
        ...
        for x in soup.select('#main-content [href*=monthly-diagnostics-data]', limit=3):
